...
/**
 * Created by AGGARWAL'S on 9/13/2015.
 */
public class PlaceMapActivity extends FragmentActivity implements OnMapReadyCallback {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Lati = Double.parseDouble(getIntent().getStringExtra(ChoiceDisplayAdapter.INTENT_LATI));
        Longi = Double.parseDouble(getIntent().getStringExtra(ChoiceDisplayAdapter.INTENT_LONG));
        place_name = getIntent().getStringExtra(ChoiceDisplayAdapter.INTENT_PLACE_NAME);
        place_id = getIntent().getStringExtra(ChoiceDisplayAdapter.INTENT_PLACE_ID);
        color = getIntent().getIntExtra(ChoiceDisplayAdapter.INTENT_COLOR, 0);
    ...
    private class HttpGetDirections extends AsyncTask<Void,Void,Void>{
        ...
        HttpGetDirections(GoogleMap map){
            text = null;
            GPSGetLocation gpsGetLocation = new GPSGetLocation(PlaceMapActivity.this);
            URL = "https://maps.googleapis.com/maps/api/directions/json?origin="+
                    gpsGetLocation.getLatitude()+","+gpsGetLocation.getLongitude()+"&destination=place_id:"
                    + place_id+"&key=AIzaSyBAuY7uwzJkS1d1Cp8WLYphhs4UuAZ7ZL4";
            googleMap = map;
        }
        ...
        protected void onPostExecute(Void aVoid) {
            ...
            options.color(getResources().getColor(color)).width(7);
